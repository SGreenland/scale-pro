<template>
  <div class="flex w-fit items-center m-auto gap-4 px-6 pb-3">
    ...
    <button
      ...
      :class="{ '  ': audioIsPlaying }"
      @click="$emit('toggleAudio', isForwardsAndBackwards, shouldLoop)"
    >
      <font-awesome-icon :class="audioIsPlaying ? '' : 'ml-0.5'" :icon="audioIsPlaying ? faStop : faPlay" />
    </button>
    <div
      ...
      :class="{ 'text-cyan-600 dark:text-cyan-300': isForwardsAndBackwards }"
    >
